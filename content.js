// 🎮 Steam Friends Tracker - Fixed
let checkInterval = null, hasInitialized = false, lastNotificationTime = 0;
...
function getFriendsFromPage() {
  ...
  containers.forEach(container => {
    ...
    if (name === 'Unknown' || !name) name = container.getAttribute('aria-label') || overlay?.getAttribute('title') || steamId;
...
function checkAndSave() {
  ...
  if (currentFriends.length === 0 || hasInitialized) return;
  
  chrome.storage.local.get(['lastFriendsList', 'logs'], (result) => {
    const lastList = result.lastFriendsList || [], logs = result.logs || [];
    const lastIds = new Set(lastList.map(f => f.id)), currentIds = new Set(currentFriends.map(f => f.id));
...
function startTracking() {
  ...
  const isMainFriendsPage = currentUrl.includes('/friends') && !currentUrl.includes('/friends/pending') && !currentUrl.includes('/friends/outgoing') && !currentUrl.includes('/friends/blocked');
  ...
  const observer = new MutationObserver(() => {
    ...
    if (document.querySelectorAll('div.friend_block_v2[data-steamid]').length > 0 && !hasInitialized) checkAndSave();
  });
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', startTracking);
else startTracking();

window.addEventListener('load', () => setTimeout(startTracking, 3000));
chrome.runtime.onMessage.addListener((msg) => { if (msg?.type === 'MANUAL_CHECK') { hasInitialized = false; checkAndSave(); } });